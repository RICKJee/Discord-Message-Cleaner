...
LOG_DIR = Path.home() / "Documents/Random Python/Message Cleaner/logs"
...
# ---------------- Redirect stdout to GUI and log file ----------------
...
def show_about():
    ...
    tk.Label(about, text="Created by Random Python Discord", font=("Segoe UI", 12), bg="#2f3136", fg="#dcddde").pack(pady=10)
...
def setup_tray():
    icon_path = resource_path("Cleaner_icon-icons.com_53211.ico")
    image = Image.open(icon_path)
    menu = (item('Show Window', lambda: window.deiconify()), item('Quit', on_close))
    global tray_icon
    tray_icon = pystray.Icon("Message Cleaner", image, "Message Cleaner", menu)
...
def create_gui():
    ...
    tk.Button(button_frame, text="Open Logs", command=open_logs, bg="#4f545c", fg="white").pack(side='left', padx=5)
    tk.Button(button_frame, text="Delete Logs", command=delete_logs, bg="#4f545c", fg="white").pack(side='left', padx=5)
    ...
    window.protocol("WM_DELETE_WINDOW", lambda: [window.withdraw(), print("🧊 Minimized to tray")])
    setup_tray()
    window.mainloop()
